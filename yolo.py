...
from utils.dictionary import load_dict, load_class
...
class YOLO(object):
    ...
    def __init__(self, **kwargs):
        # 重要配置信息
        self.model_path = './model/ver2.0.pth'
        self.dict_path = './model/dict.npy'
        self.dict_path = os.path.expanduser(self.dict_path)
        self.model_image_size = (416, 416, 3)
        self.confidence = 0.2
        self.iou = 0.1
        self.cuda = True

        self.class_names = load_class(filename=self.dict_path)
        self.config = Config
        self.generate()
    ...
    def detect_image(self, image):
        ...
        for i, c in enumerate(top_label):
            ...
            print('label = {}, position = {}, {}, {}, {}'.format(label, top, left, bottom, right))
            ...
            for i in range(thickness):
                draw.rectangle(
                    ...
                    outline=self.colors[self.class_names.tolist().index(predicted_class)])
            draw.rectangle(
                [tuple(text_origin), tuple(text_origin + label_size)],
                fill=self.colors[self.class_names.tolist().index(predicted_class)])
