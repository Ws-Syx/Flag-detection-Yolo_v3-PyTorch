...
class YoloDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        # 提取各个框的信息 size(box): [num_of_boundinBox, 5]
        ...
        if not random:
            ...
            # NEAREST: 从输入图像中选择一个最近的像素。忽略所有其他输入像素。
            # BILINEAR: 若要调整大小，请对所有可能影响输出值的像素使用线性插值计算输出像素值。
            # BICUBIC: 若要调整大小，请对所有可能影响输出值的像素使用三次插值计算输出像素值。
